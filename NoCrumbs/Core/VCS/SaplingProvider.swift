...
struct SaplingProvider: VCSProvider {
    ...
    private static let resolvedPath: String = {
        VCSBinaryResolver.resolve("sl", knownPaths: [
            "/opt/facebook/hg/bin/sl",
            "/opt/homebrew/bin/sl",
            "/usr/local/bin/sl",
        ])
    }()
    ...
    func currentBranch(at path: String) async throws -> String {
        try await run(args: ["bookmark", "--active"], at: path)
    }
        ...
        try await withCheckedThrowingContinuation { continuation in
            DispatchQueue.global().async {
                ...
                if process.terminationStatus == 0 {
                    ...
                    continuation.resume(throwing: VCSError.commandFailed("sl", process.terminationStatus, stderr))
                }
            }
        }
    }
